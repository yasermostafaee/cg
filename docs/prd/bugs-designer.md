...
## [~] B-051 — Path Style controls do nothing on pen paths (fill / stroke / width / dash silently no-op) ⟨priority: high⟩ — fixed on `fix/pen-path-style-and-layer-icon` (change dir `openspec/changes/fix-pen-path-style-commits`); pending owner verification
...
## [~] B-052 — pen/path layer shows the rectangle icon in the timeline layer list ⟨priority: low⟩ — fixed on `fix/pen-path-style-and-layer-icon` (same branch as B-051); pending owner verification
