...
## [~] B-035 — composition not fit-to-canvas on project / template open ⟨priority: medium⟩ — fixed on `fix/B-035-fit-on-open`. ROOT CAUSE: the fit-on-open `useLayoutEffect` (`CanvasArea.tsx`) was keyed on `[sceneId, resolution]` only and read `outerRef.clientWidth/Height` synchronously; on a cold open (canvas not yet laid out) the viewport was 0 → `fitZoom` returned `null` (`geometry.ts:373-374`) → `fitToViewport` silently no-opped, and since the effect didn't depend on the viewport size it never retried once the ResizeObserver measured it. FIX: extracted a pure `fitOnceForScene` gate (`fit-on-open.ts`) — fit fires exactly once per scene, gated on scene-ready AND viewport-measured; `fitToViewport` now returns whether it actually fit and the scene is marked fitted ONLY on a real fit, so a warm synchronous attempt (no flash) and a cold viewport-gated retry share one guard with no double-fit and leave a manual zoom alone. Unit test `fit-on-open.test.ts` (warm/cold handoff) + E2E `fit-on-open.spec.ts` (project + template open are fit without a manual Fit).
