...
## [~] D-122 — snap dragging to integer scene pixels while the pixel grid is active ⟨priority: medium⟩ — implementing on `feat/D-122-pixel-snap-drag` (`openspec/changes/add-pixel-snap-drag`): pixel snap for drag + arrow-nudge at grid zoom (direction-aware first-snap to the next integer), Alt bypass, Snapping-preference master switch; below-threshold + Inspector values stay free. Resize-handle snapping deferred (noted in `design.md`)
