...
## [~] D-125 — import After Effects → bodymovin (Lottie) animations as a lifecycle-aware element ⟨priority: medium⟩ — change dir: `openspec/changes/lottie-lifecycle-element/`; shipping in phases — Phase 1 (import + render, #335 + canvas fixes #337/#338/#339) and Phase 2 (the lifecycle crux: OUT mapping, `drivesHold`, element-outro seam, B-034 hidden-inert gate) are IN; Phase 3 covers field overrides, the auto-exit boundary (tasks 7.6), the E2E, and the CasparCG 2.3.x CEF hardware smoke (the pre-archive gate)
