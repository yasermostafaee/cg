...
## [~] D-124 — path point-edit MODE: single-click selects (box only), double-click edits; Ctrl-gated point insertion ⟨priority: medium⟩ — implemented on `fix/pen-edit-mode-and-bbox` (change dir `openspec/changes/fix-pen-edit-mode-and-bbox`); pending owner verification
