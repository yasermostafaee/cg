...
## [x] D-046 — Sizing=auto guard (confirm modal + squeeze off + no keyframes on text metrics) ⟨priority: high⟩ — archived: `openspec/changes/archive/2026-06-29-consume-fitmode-auto-size-text/` (shipped with D-060, #223). Warn+confirm + auto→fixed one-shot size commit. Squeeze-off out of scope (autoSqueeze unimplemented).
...
## [x] D-060 — Auto-size text rendering (consume fitMode) ⟨priority: medium⟩ — archived: `openspec/changes/archive/2026-06-29-consume-fitmode-auto-size-text/` (consumes fitMode; covers D-046, #223)
...
## [x] D-110 — Path morphing (per-point keyframe animation) ⟨priority: medium⟩ — merged (#284, `68bc438`), archived (`openspec/changes/archive/2026-07-13-add-path-morph`): AE-style whole-shape keyframes on a single Path timeline row — each keyframe snapshots the full anchor array, the anchor set is invariant across a path's keyframes, and selection bounds follow the live morphed shape. Pinned by `store-path-morph.test.ts`, `e2e/path-morph.spec.ts` and `import-path-morph-vcg.test.ts` (the morph survives a `.vcg` round-trip). Living spec: `designer-path-element` (4 new requirements + the animate-like-a-shape requirement modified)

**What:** Make a `path` element's outline shape animatable. Model it as a single animatable `path` property (After-Effects style): each keyframe stores a full snapshot of the anchor array, and the runtime interpolates point-by-point (anchor x/y + in/out handles) between two path keyframes, matching anchors by their stable `id`. The editor GUARANTEES every keyframe shares one anchor set (AE-style structure lock — structural edits propagate to all keyframes); hold/snap on mismatched sets is only the runtime's defensive path for external/malformed data.
...
**Acceptance (owner-decided 2026-07-10):**

- WHEN the operator selects a path element THEN the timeline shows a single **Path** property row (alongside Transform / Path style / Filter) — one lane of diamonds for the whole shape, never one row per anchor
- WHEN the operator adds the first Path keyframe (diamond/stopwatch, same affordance as every property) THEN the keyframe stores a snapshot of the full anchor set (ids, x/y, in/out handle deltas) evaluated at the playhead
- WHEN the Path property is keyframed and the operator edits points at a frame with no Path keyframe THEN a new keyframe is auto-recorded at that frame with the edited snapshot (exactly the transform auto-record rule); editing ON a keyframe's frame updates that keyframe
- WHEN the playhead sits between two Path keyframes THEN each anchor present in BOTH snapshots (matched by stable id) tweens its x/y and its in/out handle deltas along the segment's easing (`step` / named presets / custom cubic-bézier — same easing machinery as every other track)
- WHEN the operator inserts or deletes an anchor on a keyframed path THEN the structural edit applies to the static base and EVERY keyframe (insert: one shared new id at the same parametric t on each keyframe's own segment; delete: the id removed everywhere; below-2 deletes the element) so the anchor set never desyncs and the new anchor tweens — while move/handle edits still record only the current frame (owner-decided 2026-07-11)
- WHEN a keyframed path morphs THEN the selection box and the Inspector W/H follow the LIVE interpolated outline at the playhead — growing and shrinking with the shape, correct under rotation/scale — while other element kinds keep their existing bounds behavior (owner-decided 2026-07-11)
- WHEN external/hand-edited data carries mismatched anchor sets THEN the runtime degrades honestly (no crash, no tween for unmatched ids) and preflight warns — the editor itself can no longer author this
- WHEN a scene with a Path track is saved, reloaded, exported to `.vcg` and single-file HTML, and played by the runtime THEN the morph evaluates identically everywhere (preview == export) and ascending-frame validation holds
  **Notes:** Anchor `id` is the correspondence key; registry (D-051) gets a special path descriptor; runtime feeds the interpolated point set into the existing `pathD` builder; closed/fill and the size==visualBBox model hold while points animate. Phase 2 (smooth morphing across genuinely different point counts) stays deferred.
...
- WHEN a content element with `repeat: 'infinite'` participates in the hold (`drivesHold !== false`) THEN the checklist row shows a clear inline warning that this element makes the hold run until stop (i.e. the graphic won't auto-close)
- WHEN the nested-content indicator (D-108) includes an infinite-repeat driver THEN the same warning surfaces there (read-only), pointing the operator to the composition where it lives
- WHEN every hold driver of a composition is infinite-repeat (so the hold can never end on content) THEN the warning is prominent at the hold-source/checklist level, not just per-row
- WHEN the operator excludes the infinite-repeat element (`drivesHold === false`) or sets a finite `repeat` THEN the warning clears
...
- WHEN a parent nests a composition instance containing content (ticker / sequence / countdown clock) THEN the parent's hold checklist lists that nested content with a WRITABLE toggle (not read-only)
- WHEN the operator toggles a nested row OFF in the parent THEN that element no longer drives THIS parent's hold, stored as a per-instance override on the parent's composition-instance element — NOT on the shared child element
- WHEN the same child is instanced in another parent THEN that other parent's hold is UNAFFECTED by the first parent's override (per-instance isolation)
- WHEN a nested element has no per-instance override THEN its effective participation falls back to its own `drivesHold` (absent ⇒ drives, `false` ⇒ excluded) — backward compatible
- WHEN the parent excludes the nested looping element and keeps the finite one THEN the parent holds until the finite content completes then plays its outro (the looping element keeps running visually until the parent's exit) — matching single-composition behavior
- WHEN a hold-driving row (own or nested) is infinite-repeat (`repeat: 'infinite'`, effectively ON) THEN an inline warning states the graphic won't auto-close; excluding it or making it finite clears it (folded-in D-111)
- WHEN a scene with per-instance overrides is saved, reloaded, previewed, exported THEN it round-trips and validates and the overrides survive

**Notes:** SUPERSEDES D-111 (don't implement D-111 separately). Builds on D-108 (`apps/designer/src/renderer/features/inspector/PlayoutSection.tsx` + `nestedHoldGroupsOf`) and B-031 (runtime nested content-wait aggregation). HIGH-RISK — playout aggregation + schema → RECON FIRST.
Schema (`@cg/shared-schema`): optional `holdOverrides?: Record<string, boolean>` on the composition-instance element (`type: 'composition'`, the one with `compositionId`), keyed by the nested content element's stable id; value = whether it drives THIS parent's hold. Absent ⇒ fall back to the element's `drivesHold` (non-breaking, round-trip stays green, no version bump). Scope each override to the referenced comp's OWN content (recursing containers); deeper composition-instances carry their OWN `holdOverrides` (cascade per instance level, matching D-108's progressive drill). Exact resolution order + deep-nesting scheme → record in `design.md`.
Runtime (`@cg/template-runtime`): in the parent's aggregation (B-031's `whenSettled` / `contentTreeWait` path), effective "drives THIS parent's hold" for a nested element = the instance's `holdOverrides[id]` if defined, else the element's `drivesHold`. The element's OWN `drivesHold` still governs the child's OWN coordinator hold — the instance override ONLY affects the parent's aggregation. No animation-engine change.
...
- WHEN the operator clears the out-point while `mode` is `auto-out` THEN `mode` reverts to `manual` in the same store action (single undo entry)
- WHEN the operator clears the out-point while `mode` is `loop-cycle` THEN `mode` reverts to `manual` likewise
- WHEN the operator clears the out-point while `mode` is already `manual` THEN nothing changes (no spurious mode write)
- WHEN `mode` reverts to `manual` THEN the Playout inspector + timeline reflect it immediately, and the `NEEDS_OUTPOINT` modes show disabled (consistent with the no-out-point state)
...
## [x] D-121 — bundle font files in `.vcg` export (fix content-driven crawl-duration drift) ⟨priority: medium⟩ — merged (#298, `0ea0edf`), archived (`openspec/changes/archive/2026-07-13-bundle-fonts-in-vcg-export`): the Exporter resolves each `scene.fonts` entry's bytes into `pack()`'s existing `fonts` seam (`fonts/` dir + `kind: 'font'` assetIndex entry) and bakes a package-relative `@font-face` per bundled face into the package's `index.html`, so an unzipped `.vcg` renders the correct face under CEF with no external/`file://` request and a content-driven ticker measures real glyph widths → the crawl-derived hold duration matches the authoring machine. A face with no shippable bytes (`system`/licensed) is skipped and `vcg-ticker-fonts-not-bundled` is re-scoped to fire only for that font (no longer merely because a scene has a ticker). Single-file HTML export unchanged; no `@cg/vcg-format`, Runtime, or bridge change. New living spec: `openspec/specs/designer-font-export/`

**What:** Include the template's font files inside the `.vcg` package (today `.vcg` exports omit them — only single-file HTML inlines fonts). This makes `.vcg` portable AND fixes a correctness issue: a content-driven ticker measures text width to compute when the crawl completes one pass (which sets when the hold ends), so on a machine missing the font the crawl measures fallback glyphs → wrong width → wrong content-driven duration on air.
**Why:** A `.vcg` is meant to run anywhere, but a crawl's timing silently depends on fonts that aren't shipped — same class of "a playout-critical input is lost in export" gap as B-032's holdMs. The current state forces operators to the single-file HTML export to get correct crawl timing (the live `vcg-ticker-fonts-not-bundled` preflight warning says exactly that); bundling fonts in `.vcg` removes the footgun and the warning's reason to exist.
**Acceptance:**

- WHEN a template using non-system fonts is exported as `.vcg` THEN the font files are bundled in the package (the `@cg/vcg-format` packer already accepts a `fonts` byte map and writes a `fonts/` dir — the Exporter must supply the bytes) and the package CSS references them from within the package
- WHEN that `.vcg` runs on a machine WITHOUT those fonts installed THEN text renders with the correct font and a content-driven ticker's crawl measures the correct width → the content-driven hold duration matches the authoring machine
- WHEN the fonts are bundled THEN the `vcg-ticker-fonts-not-bundled` warning no longer fires (or downgrades to informational); it still fires for a font that genuinely can't be bundled
- WHEN exported and re-opened / run THEN the package round-trips and validates; single-file HTML export behavior is unchanged (it already inlines fonts)
...
whole scene pixels at **every** zoom — more precisely, the gesture QUANTISES ITS POINTER there, which
is not the same promise; the exceptions are enumerated in [[B-180]] (corrected 2026-08-27) — and
resize-handle snapping (deferred by this item) ships with it. Everything else this item decided is intact and re-asserted by [[B-180]]'s own tests: **`Alt` is
...
- ~~WHEN the zoom is below the grid threshold THEN drag and nudge behave exactly as today (no new snapping)~~ — **superseded by [[B-180]]:** below the threshold the NUDGE still behaves exactly as this bullet says, but a DRAG or RESIZE now quantises its pointer to a whole scene pixel there too
...
## [x] D-123 — right-click anchor context menu on paths (Delete point) ⟨priority: medium⟩ — merged (#275, `c47e3bf`), archived (`openspec/changes/archive/2026-07-10-add-anchor-context-menu`): right-click an anchor in edit mode → accessible menu (focus-first, Arrow wrap, capture-owned Esc, wheel dismiss) whose Delete point reuses the keyboard-delete semantics (re-stitch; below-2 deletes the element; one undo). Owner-verified; per owner feedback the chrome was restyled onto the shared `ui/ContextMenu.css.ts` in B-058 (#280) and the menu now opens inside D-124's point-edit mode
...
**Notes:** Implemented in `PathEditor.tsx` (right-click wiring on the anchor rects ONLY — handles/segments untouched; `dragAnchor` now ignores non-primary buttons) + a minimal `AnchorContextMenu` in the canvas feature — no shared menu primitive exists (the timeline's `LayerContextMenu` is bespoke and mouse-only), so the new component follows that pattern (fixed backdrop + clamped `role="menu"`) and adds the keyboard support (focus-first, Arrow wrap, Esc via capture-phase stop — the same mechanism as PathEditor's Delete pre-empt). The deletion logic is the EXISTING `removeAnchor` — not duplicated. App-wide native-menu suppression (App.tsx) already covers the no-native-menu requirement; the anchor handler preventDefaults locally as defense in depth. Spec: `## MODIFIED` on `designer-path-element` ("A selected path is fully editable"). Tests: unit `apps/designer/tests/anchor-context-menu.test.ts` (wiring, below-2 branch, Esc ownership via a bubble-phase spy, wheel dismissal, Arrow focus); E2E `apps/designer/tests/e2e/anchor-context-menu.spec.ts` (menu → delete → re-stitch; undo restores; below-2 deletes the element; Esc keeps selection + tool). FOLLOW-UP: the menu chrome was restyled onto the shared `ui/ContextMenu.css.ts` in B-058 (owner feedback on this item's preview), and the menu now opens in D-124's point-edit mode.

## [x] D-124 — path point-edit MODE: single-click selects (box only), double-click edits; Ctrl-gated point insertion ⟨priority: medium⟩ — merged (#280, `b841ebe`), archived (`openspec/changes/archive/2026-07-10-fix-pen-edit-mode-and-bbox`): single click = box only, double-click = point edit (`editingPathId`, gizmo hidden), Esc/empty-click exits keeping selection; Ctrl/Cmd-gated segment insertion; segment right-click Add point / Add curve point (Issue D). Owner-verified with the whole expanded Prompt-11 set

**What (owner redesign, decided with Prompt 11):** unify paths with rect/ellipse — a single click selects a path and shows ONLY the selection box; the anchor/handle overlay (`PathEditor`) appears only after DOUBLE-clicking the shape ("point edit mode", state `editingPathId` — the `editingTextId` pattern), during which the transform gizmo hides (a dedicated mode, like inline text editing — also avoids the B-037 hijack class over the anchors). Esc OR a click on empty pasteboard exits back to plain selection (selection KEPT; the next Esc/empty click deselects as before); selecting anything else, another path's edit mode, a composition switch, or a tool switch clears the mode. Point INSERTION becomes intentional: the segment add-point affordance (copy cursor + hit surface) exists only while **Ctrl** (or Cmd on macOS) is held, and insertion (click = corner, drag = smooth — B-056/B-057 semantics) fires only then; without the modifier a segment click does nothing special.

**Esc precedence (spelled out in the change's `design.md`):** anchor-menu close (capture-owned) → editable/bind/text-edit owners → pen branch (drawing cancel / idle exit) → point-edit exit → deselect. Enforced structurally in the single CanvasOverlay dispatcher — no new window listeners.

**Acceptance:**

- WHEN a path is single-clicked with the select tool THEN it selects with the box only (no anchors/handles)
- WHEN a path is double-clicked THEN point-edit mode opens (anchors/handles; gizmo hidden) and all anchor interactions apply (drag anchor/handle, Ctrl-insert, right-click Delete point)
- WHEN Esc or an empty-pasteboard click happens in edit mode THEN the mode exits with the selection kept; the next Esc/empty click deselects
- WHEN Ctrl/Cmd is held in edit mode THEN segments show the add-point affordance and click/drag inserts a corner/smooth point; without it segment clicks are inert
- WHEN another element is selected, a composition or tool changes THEN the mode clears
...
- WHEN a bodymovin `.json` (or `.lottie`) is imported into the project assets THEN it validates through `@cg/lottie-bridge`'s allowlist (3D layers / expressions / effects / audio rejected with a readable reason), lands as a `lottie` asset, and a `lottie` element can be created and placed on the canvas
- WHEN a `lottie` element is selected THEN it can be positioned / scaled / rotated / opacity-animated and timed on the timeline like any other element, and the Inspector exposes speed, hold behavior and the phase mapping — but NOT the animation's internal keyframes (opaque by design)
- WHEN the imported JSON carries bodymovin `markers` THEN intro-end / outro-start (and an optional idle segment) are read from them; WHEN it does not THEN the operator marks them by hand in the Inspector against the animation's own frame space
- WHEN the composition plays THEN the Lottie plays its intro ONCE and then holds at the hold frame — freezing, or looping the mapped idle segment — instead of looping the whole animation
- WHEN a native ticker/sequence sits ON TOP of the Lottie in a `content-driven` composition THEN the ticker drives the hold (existing model) and the Lottie holds beneath it; the Lottie does NOT drive the hold by default (`drivesHold`-style opt-out) and can be opted IN
- WHEN the composition is stopped (`stop()`) or exited (`out()`) THEN the Lottie plays its OUTRO segment, the background outro does not close over it, and the composition settles to CLEARED with every driver halted — consistent with D-085 and D-105's content-first / background-last order
- WHEN the scene is paused and resumed THEN Lottie playback freezes and continues in lockstep with the rest of the scene (no drift against the `FrameDriver` playhead)
- WHEN a Lottie field override (text / color; image if cheap) is bound THEN it resolves through the EXISTING fields/bindings model and the `lottie-override` target stops being a no-op — a secondary path here, since the native overlay carries the dynamic content
...
> composition's in-point.** Linux `e2e` for the WHOLE item (both halves) is DISCHARGED for
> `710f0ab0` — <https://github.com/yasermostafaee/cg/actions/runs/31683317925> (`success`, and
> the `E2E (Playwright)` job RAN); this supersedes the runs for `d43a9adb`, `f9a35507`,
> `0d2d0c3b`, `f492939d` and `6e620f70`, each of which verified a tree that has since been
> replaced.
> The in-point clause
