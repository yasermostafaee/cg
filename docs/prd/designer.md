...
## [ ] D-119 — bundle font files in `.vcg` export (fix content-driven crawl-duration drift) ⟨priority: medium⟩
...
**Why:** A `.vcg` is meant to run anywhere, but a crawl's timing silently depends on fonts that aren't shipped — same class of "a playout-critical input is lost in export" gap as B-032's holdMs. The current state forces operators to the single-file HTML export to get correct crawl timing; bundling fonts in `.vcg` removes that footgun and the warning's reason to exist.
**Acceptance:**

- WHEN a template using non-system fonts is exported as `.vcg` THEN the font files are bundled in the package (alongside cg.css/cg.js/template.json)
...
- WHEN the bundled fonts are present THEN the "ticker element(s) but .vcg doesn't bundle font files" warning no longer fires (or downgrades to informational); the warning still fires if a font genuinely can't be bundled
- WHEN exported and re-opened / run THEN it round-trips and validates; single-file HTML export behavior is unchanged (it already inlines fonts)
...
**Notes:** Export/packaging change — the `.vcg` writer (`apps/designer/src/platform/Exporter*` / the `@cg/vcg-format` packer) bundles the font assets and cg.css references them from within the package; the runtime/loader resolves fonts from the bundle. Mirror how single-file HTML already inlines/embeds fonts (reuse that asset path). Mind: font file size in the package, format (woff2 preferred), and the licensed-font edge. This closes the warning shown in the preflight (the same `findFiniteTicker`-adjacent diagnostic surface). Spec: `## ADDED`/`## MODIFIED` the export capability (`.vcg` now bundles fonts). Tests: an exporter test (a non-system-font template produces a `.vcg` containing the font asset + cg.css reference); a render/measurement test or fixture asserting crawl width/duration is font-correct without the system font; the warning-suppression case.
