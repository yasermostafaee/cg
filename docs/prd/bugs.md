...
## [ ] B-031 — a content-driven nested composition does not drive its parent's hold, so the parent never closes on the nested content ⟨priority: high⟩ — D-104 follow-up (distinct from B-030)
...
## [ ] B-032 — timed hold (`holdMs`) is ignored for a content-less auto-out / loop-cycle composition; any value behaves like 0 ⟨priority: high⟩

> A composition with NO content source (no ticker / sequence / countdown), set to `auto-out` or `loop-cycle`
> with a timed hold, closes almost immediately after its intro regardless of `holdMs` — 10000 ms behaves like
> 0. Reproduces on a freshly-created composition (not a leftover content-driven one).

**Repro:**

1. Create a NEW composition with a simple entrance animation and an out-point on the timeline (no ticker/sequence/countdown).
2. In the preview timing, set mode to `auto-out` (or `loop-cycle`) and set the hold (`holdMs`) to a large value, e.g. 10000.
3. Play it (preview modal / exported single-file HTML).

**Expected:** after the intro, the graphic HOLDS for `holdMs` (≈10 s) before playing its outro.
**Actual:** it plays the outro almost immediately — `holdMs` has no effect; any value equals 0.
**Env:** Browser + Designer preview + exported single-file HTML (runtime `@cg/template-runtime`; controller `playout-controller.ts`; preview timing `features/fields/PreviewTimingControls.tsx` + override threading via `PreviewModal.tsx` → `platform/preview.ts`).
**Investigation note:** static reading suggests the timed path SHOULD work — a content-less comp resolves `holdSource: 'timed'` (`playoutOf`), `onIntroEnd` takes the else-branch `scheduleHold(holdMs ?? 0, startOutro)`, and the override threads `overrides[''] → playoutOverride → createRuntime → effectivePlayoutFor`. So the defect is subtler (a regression, a dropped override field, a stale-token/clock interaction in the preview iframe, or the comp resolving to content-driven despite no content). RECON FIRST: reproduce, log the resolved `effPlayout` + the controller branch actually taken, then fix the real cause.
**Regression test:** a `@cg/template-runtime` controller test (injectable clock) asserting a content-less `auto-out` comp with `holdMs: N` holds ≈N ms before the outro; plus a check that the preview's `holdMs` override reaches `effectivePlayoutFor`. Capability: `designer-playout-lifecycle`.