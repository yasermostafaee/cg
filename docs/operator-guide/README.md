...
The `AUDIT` button in the StatusBar opens a tail of the NDJSON log. Filter by action (load / take / lock-engage / failover / …) or actor. On a station that signs in, the actor is the person the bridge verified. On one that does not, a row a console caused reads `console` — somebody at a console did it, and nobody signed in — and a row the station caused by itself (boot recovery, reconciliation) reads `unattributed`. There is no name to type and nothing is hidden by that. The log is append-only and forensic — a clean exit is recorded; a crash leaves the file partially written but the next boot picks up where it left off.
