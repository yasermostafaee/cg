...
| `PLAYOUT-2.8.54-CG-FACTS-2026-09-23.md` | 🔴 **Playout 2.8.54 (2026-09-23): the built-in `cg-admin` (random password per install, read from the Playout's settings), the fixed `iss` `urn:apasai:playout` — NOT an install identity, the root of trust is the JWKS at the entered address — the loopback `casparHost`, and AMCP AUTO-TRUST** (a server-side `station-admin` D4/D8/D9 read opens 5250 to its source IP; 7-day expiry; no NAT or proxy). What CG Control does about each. |
