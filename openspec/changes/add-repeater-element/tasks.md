...
- [x] 2.1 `RepeaterElementSchema` (`type: 'repeater'`; required
...
- [x] 2.2 `repeater-items { elementId }` variant in `BindingTargetSchema`
- [x] 2.3 Schema unit tests: defaults + round-trip; invalid gap/maxItems
      rejected

## 3. Runtime (@cg/template-runtime)

- [x] 3.1 `buildRepeater` in `scene-builder.ts`: clipped outer box;
...
- [x] 3.2 `repeater-driver.ts`: `RepeaterDriver`
...
- [x] 3.3 `runtime.ts`: instantiate RepeaterDrivers per scope inside
...
- [x] 3.4 `bindings.ts`: `applyOne` case `repeater-items` → driver
      `setItems` (structured value, no stringify/transform)
- [x] 3.5 Unit tests: build-time authored stamp; fresh-play stamp from
...
- [x] 3.6 Doc-sync: `packages/template-runtime/README.md` (RepeaterDriver +
      `wireScopeSubtree` as the extension seam; wiring-tree vs
      namespace-tree distinction); `docs/engines/overview.md` if needed

## 4. GDD (@cg/vcg-format)

- [x] 4.1 `gdd.ts`: a `list` field bound `repeater-items` derives its item
...
- [x] 4.2 vcg-format unit tests: derived schema (types/constraints/
      required); non-repeater lists unchanged

## 5. Designer UI

- [x] 5.1 Repeater tool `{ id: 'repeater', label: 'Repeater', icon: '▤' }`
...
- [x] 5.2 `RepeaterSections` in `StyleSection.tsx`: Composition select
...
- [x] 5.3 `ListItemsEditor`: `columns: {key,label,kind?}[]` prop (the
...
- [x] 5.4 Data key: `setElementDataKey` repeater branch (list seeded from
      ...
      UNCHANGED
- [x] 5.5 Designer unit tests: defaultRepeater; columns derivation from
      child fields; insertion guard (no valid composition ⇒ no insert)

## 6. E2E + gate

- [x] 6.1 `apps/designer/tests/e2e/repeater.spec.ts` (+ fixtures): child
      ...
      `pnpm test:e2e`
- [x] 6.2 Green gate per CLAUDE.md (format:check + typecheck + lint + test + build), test task uncached once (`turbo --force`);
