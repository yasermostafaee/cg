...
- [x] Copy the 11 woff2 into `apps/runtime/public/fonts/**` and `fonts.css` into
      `apps/runtime/src/renderer/fonts.css` (same faces the Designer ships).
- [x] `produceTemplateDelivery` / `importTemplateFromBytes` accept a `fontsCss` +
      `fetchUrl` options object; pass them through to the exporter.
- [x] `LibraryPanel` imports `fonts.css?inline` and passes it as `fontsCss` (default
      `fetchUrl` = browser `fetch`, resolving `/fonts/…` from the SPA).
- [x] Test: with the real `fontsCss` + a node-fs `fetchUrl`, the produced HTML
      inlines the bundled `@font-face` faces as base64 and has **no external refs**.

## Part 2 — bridge HTTP serve

- [x] Add `tools/caspar-bridge/src/template-http-server.ts` — `TemplateHttpServer`
      over `getHtml(id)`: `GET /template/<id>` → `200 text/html; charset=utf-8`,
      `404` unknown; `start(opts)`, `stop()`, `urlFor(id)`, `guessLanHost()`.
- [x] `CasparRuntime` owns it: derive serve opts from `connection.servers.A.host`
      (loopback local; opt-in routable + guessed/configured serve-host remote);
      `startServing()` + stop in `stop()`; expose the serve address.
- [x] `createBridge` passes serve overrides (`bindHost`/`port`/`serveHost`), awaits
      `startServing()`, logs the chosen URL; control WS stays `127.0.0.1`.
- [x] Tests: serve returns stored HTML for a known id, `404` unknown, replace on
      re-import.

## Part 3 — CG ADD the real URL + real fields

- [x] `CasparRuntime.load` builds the `CG ADD` template arg as the served
      `/template/<id>` URL when serving, else the bare id; fields pass through.
- [x] `LibraryPanel.loadOntoStack` seeds `fields` from field-schema defaults
      (`defaultFieldValue`) instead of `{}`; operator edits still flow via update.
- [x] Test: load builds the right `CG ADD` URL + the real (non-empty) field JSON.

## Part 4 — amcp-mock regression (so this bug can't hide again)

- [x] `tools/amcp-mock` `handleCg` `ADD`: resolve the template arg (URL → HTTP
...
- [x] Update the mock unit test for the new semantics (bare ref `404`; served URL
      `202`).
- [x] New integration test: bridge (with HTTP server) + hardened mock — load/take/
...
- [x] Pin the Runtime import E2E fixture's `__CG_BRIDGE_URL__` to a dead port so a
      real bridge on `127.0.0.1:5280` can't make the spec go live and flake.

## Gate

- [x] Full green gate UNCACHED (`turbo … --force`) for the touched workspaces
      (`apps/runtime`, `tools/caspar-bridge`, `tools/amcp-mock`, `@cg/shared-ipc`):
      `format:check` + `typecheck` + `lint` + `test` + `build`.
- [x] Full `pnpm test:e2e` (Designer must stay green; the import spec now pinned).
- [x] `pnpm openspec validate serve-template-and-render --strict`.
- [x] Commit + push + open a PR. **B-038 stays `[~]`.**
