...
- [x] `packages/caspar-client/src/amcp/escape.ts`: `escape()` escapes `"` → `\"`
...
- [x] `tools/caspar-bridge/src/command-builder.ts`: confirm both `CG ADD` and
...
- [x] `tools/amcp-mock/src/amcp-parser.ts` `readQuoted`: un-escape **only** `\"`→`"`;
...
- [x] `escape.test.ts`: exact wire bytes for `"`, `\` ×1–4, newline, tab, combos,
      Persian; assert backslashes are NOT doubled.
- [x] `amcp-parser` test: decoding the canonical output → original JSON; decoding the
      OLD double-escaped output is observably wrong (mock catches the regression).
- [x] Bridge → amcp-mock end-to-end integration: a field value with `"`, `\` (odd AND
...
- [x] `pnpm openspec validate fix-amcp-escaping --strict`.
- [x] Commit + push + open a PR. **B-041 stays `[~]`.**
