...
- [ ] 1.1 `ClockElementSchema` (`type: 'clock'`; base + ticker's text-styling
...
- [ ] 1.2 Schema unit tests: valid wall/countup/countdown clocks; countdown
      without target rejected; bad target shapes rejected; defaults applied

## 2. Runtime (@cg/template-runtime)

- [ ] 2.1 `clock-format.ts` (pure): longest-token-first tokenization of
...
- [ ] 2.2 `buildClock` in `scene-builder.ts`: dataset `cgElementId` +
...
- [ ] 2.3 `clock-driver.ts`: `ClockDriver` — options `{ node, mode, format,
...
- [ ] 2.4 `runtime.ts` wiring, parallel to tickers: per-scope ClockDrivers
...
- [ ] 2.5 Unit tests (injected RuntimeClock, no real rAF): formatter
...
- [ ] 2.6 Doc-sync: `packages/template-runtime/README.md` (ClockDriver
...
- [ ] 3.1 Clock tool `{ id: 'clock', label: 'Clock', icon: '◷' }` in the
...
- [ ] 3.2 `ClockSections` in `features/inspector/StyleSection.tsx` modeled
...
- [ ] 3.3 `PlayoutSection.tsx`: `hasContentElement` also true for
      countdown clocks; content-driven copy generalized ("until the
      content completes (ticker passes / countdown reaching zero)")
- [ ] 3.4 Designer unit coverage where sibling suites exist (element
      defaults / inspector smoke)

## 4. Export/GDD assertion

- [ ] 4.1 Test: a clock scene's single-file export boots clean and its GDD
      is unchanged vs. the same scene without the clock (no exporter code
      changes expected)

## 5. E2E + gate

- [ ] 5.1 `apps/designer/tests/e2e/clock.spec.ts` mapping the spec
      ...
      `pnpm test:e2e`
- [ ] 5.2 Green gate per CLAUDE.md for touched workspaces (format:check +
