...
- [x] 1.1 `numerals.ts`: `readLocalizedNumber` (number / incomplete / invalid),
      `parseLocalizedNumber`, `parseTimeOfDay`, `parseDurationMs`, `latinNumerals`, `digitSetOf`,
      `formatNumberLike`; exported from the index.
- [x] 1.2 Tests — `۱۲٫۵` / `١٢٫٥` / `12.5` → 12.5; `۱٬۲۳۴` → 1234; `۰۰:۳۰` → 30 s; `۲۰:۳۲` →
      `20:32`; the controls: `۱۲a` invalid, `۲۵:۳۲` not a time, a Latin value unchanged.

## 2. On air — `@cg/template-runtime`

- [x] 2.1 `clock-driver.ts`: `parseTimeOfDay` from the shared reader (re-exported); `resolveTimeOfDay`
      reads through it; the local regex goes.
- [x] 2.2 `bindings.ts`: a `transform` target falls back to the shared reader when `Number()` is
      not finite.
- [x] 2.3 Tests — a live countdown re-targets from `۲۰:۳۲`; red first against the ASCII regex.

## 3. CG Control

- [x] 3.1 `NumericInput`: an as-typed mode (`digits="as-typed"`) — no normalisation, the scrub
...
- [x] 4.1 `ui/typedNumber.ts` (`useTypedNumber`) — the as-typed number buffer over the one reader;
      and `RealtimeNumberInput` / `NumberField` gain an opt-in `digits="as-typed"` so the number
      field's default keeps its look and its scrub.
- [x] 4.2 The number field's default ("Value") uses the opt-in; the preview form's number field,
      the repeater number columns and the per-item dwell (a duration) use the hook.
- [x] 4.3 `validateField` reads a pattern through `latinNumerals`.
- [x] 4.4 `rebuildField` reads a text default through the shared reader when a field becomes a
      number (`۱۲` → 12, not 0).
- [x] 4.5 Tests — dom/unit for each, red first.

## 5. E2E (§2)

- [x] 5.1 CG Control, real bridge + `@cg/amcp-mock`: `۱۲۳` / `١٢٣` / `123` arrive in `CG UPDATE`
...
- [x] 5.2 Designer: number default and preview keep Persian digits; the Time preset accepts
...
- [x] 6.3 PRD: R-020 and D-130 notes; D-130 stays queued for the Designer's chrome numbers.
