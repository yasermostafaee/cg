...
- [x] 1.1 `SequenceItemSchema { id min-1, text, dwellMs?: positive int }` +
...
- [x] 1.2 `sequence-items { elementId }` variant in `BindingTargetSchema`
- [x] 1.3 Fix the `ListItemSchema` comment in `fields.ts` to cite the
      sequence's real fields (`text`/`dwellMs`)
- [x] 1.4 Schema unit tests: defaults applied + parse round-trip; invalid
      dwell/transition values rejected

## 2. Runtime (@cg/template-runtime)

- [x] 2.1 Motion mapper (pure module): edge → offscreen offset vector sized
...
- [x] 2.2 `buildSequence` in `scene-builder.ts`: dataset `cgElementId` +
...
- [x] 2.3 `sequence-driver.ts`: `SequenceDriver` — options `{ host,
...
transitionOut, transitionTiming, transitionMs, repeat, clock? }`;
      ...
      `coerceSequenceItems`
- [x] 2.4 `runtime.ts`: per-scope SequenceDrivers from `scope.sequences`;
      ...
      steps-model seam
- [x] 2.5 `bindings.ts`: `applyOne` case `sequence-items` → driver
      `setItems` via the registry (structured value, no
      stringify/transform)
- [x] 2.6 Unit tests (injected RuntimeClock): mapper per edge incl. `none`
      ...
      render
- [x] 2.7 Doc-sync: `packages/template-runtime/README.md` (SequenceDriver
...
- [x] 3.1 Sequence tool `{ id: 'sequence', label: 'Sequence', icon: '⇉' }`
...
- [x] 3.2 `SequenceSections` in `StyleSection.tsx`: pinned "Sequence"
...
- [x] 3.3 `ListItemsEditor`: prop-gated `showDwell` per-item dwell input,
      ...
      target kind)
- [x] 3.4 Data key: `setElementDataKey` sequence branch (list field seeded
...
- [x] 3.5 Preview transport: enable the existing **Next** button for scenes
      containing a sequence (replace the hardcoded `stepCount = 1`); it
      already posts `next` → preview `window.next()` → `runtime.next()`
- [x] 3.6 `PlayoutSection.tsx`: `hasContentElement` also true for
...
- [x] 3.7 Designer unit tests: preset → three-fields mapping incl. the
      Custom fallback display; `defaultSequence` validity; data-key
      seeding/lockstep

## 4. Export/GDD assertion

- [x] 4.1 Test: a sequence scene's single-file export carries the element,
      `next()` pages it (driver in the bundled runtime), and the GDD
      matches the D-028 list representation (no generator changes)

## 5. E2E + gate

- [x] 5.1 `apps/designer/tests/e2e/sequence.spec.ts` (+ `addSequence` etc.
...
- [x] 5.2 Green gate per CLAUDE.md (format:check + typecheck + lint + test + build), test task uncached once (`turbo --force`);
