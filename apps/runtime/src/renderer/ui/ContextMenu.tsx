 ...
 *
 * A menu here is a SHORTCUT, never a new capability: every item mirrors a button that is
 * already on the surface, and carries that button's `disabled` state with it. That is load-
 * bearing — the row's PLAY/UPDATE/CLEAR are disabled while the bridge link is down (R-006),
 * and a menu that let the operator issue them anyway would be a second, unguarded door onto
 * air. The menu re-uses the buttons' own gates rather than re-deriving them.
 *
...
export function ContextMenu({ items, x, y, ariaLabel, onClose }: Props): JSX.Element {
  ...
  // Keep the focused item actually focused, so Enter/Space land on it.
  useEffect(() => {
