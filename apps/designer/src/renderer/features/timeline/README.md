...
single `translateY` (see `TimelineDock.syncScroll`). The dock deliberately does
...
This spans three layers (schema → timeline UI → runtime applier) — see the "Where
features go" map in [`CLAUDE.md`](../../../../../../CLAUDE.md):

1. **Schema** — add the path to `AnimatablePropertySchema` in `animation.ts`.
2. **Timeline UI** — add a `TimelineRow` (with its `read`/`unit`/`factor`) to the
   right group in [`keyframe-helpers.ts`](keyframe-helpers.ts) so it gets a label,
   live value, diamond and lane. No `TrackRow` change is needed — it's data-driven.
...
A property added to the schema + timeline but not the applier will animate in the
dock's value readout but not on air — all three steps are required.
