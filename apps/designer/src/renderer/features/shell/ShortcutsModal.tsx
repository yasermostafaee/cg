...
const SHORTCUTS: readonly { keys: string; label: string }[] = [
  ...
  { keys: `${MOD} + C`, label: 'Copy selected layer(s)' },
  { keys: `${MOD} + X`, label: 'Cut selected layer(s)' },
  { keys: `${MOD} + V`, label: 'Paste layer(s)' },
  { keys: 'Arrow keys', label: 'Nudge selection by 1px (snaps to the pixel grid at high zoom)' },
  { keys: 'Shift + Arrow', label: 'Nudge selection by 10px' },
  { keys: 'Alt + Arrow / Drag', label: 'Bypass pixel-grid snap (free sub-pixel move)' },
  { keys: 'Shift / Ctrl + Click', label: 'Add / remove from selection (canvas or layer)' },
