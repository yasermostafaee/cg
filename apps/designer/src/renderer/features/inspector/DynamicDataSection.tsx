...
/**
 * D-018 / D-028 — "Dynamic / Data" section, shown for text AND ticker
 * elements. An element becomes a runtime data field when given a non-empty
 * **Data key**; the store's convenience layer (`setElementDataKey` /
 * `setElementFieldMeta`) keeps the scene-level `fields[]`+`bindings[]` in
 * sync. A text element syncs a full-text binding + text field; a ticker syncs
 * a `ticker-items` binding + `list` field (its items stay editable in the
 * inspector's Items section and in the preview form). This is distinct from
 * the inspector's "Name" row, which edits the element name.
 */
export function DynamicDataSection({
  element,
  scene,
}: {
  element: TextElement | TickerElement;
  scene: Scene;
}): JSX.Element {
  // The convenience binding for this element: the full-text one for text (no
  // placeholder), the items one for a ticker; its field is what the meta edits.
  const isConv = (b: FieldBinding, elementId: string): boolean =>
    element.type === 'ticker'
      ? b.target.kind === 'ticker-items' && b.target.elementId === elementId
      : b.target.kind === 'text' &&
  ...
  // A convenience binding owned by an element OTHER than this one (either
  ...
  // shape (text/multiline for text elements, list for tickers) that aren't
  // owned by another element (i.e. orphaned by an "unbind"), so the operator
  // can reconnect a field here instead of hunting for it.
  ...
  return (
    <CollapseSection title="Dynamic / Data" defaultExpanded={field !== undefined}>
      ...
      {field === undefined ? (
        <p className={s.hint}>
          {element.type === 'ticker'
            ? 'Give this ticker a Data key to drive its items from field data at playout.'
...
function FieldMeta({
  ...
  // D-028 — a ticker's list field: only the base meta applies; the items
  // themselves are edited in the Items section (and live in the preview form).
  if (field.type === 'list') {
    return (
      <>
        <TextField
        ...
        />
        <p className={s.hint}>
          The field’s items are the ticker’s Items section; operators edit them live in the
          preview’s data form.
        </p>
      </>
    );
