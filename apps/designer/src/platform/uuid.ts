 ...
 * `localhost`). When the Designer is served over plain HTTP to a LAN IP
 * (e.g. `http://192.168.x.x:4173`) the method is absent and calling it
 * throws. `crypto.getRandomValues()` has no such restriction, so we build
 * the id from it whenever `randomUUID` is unavailable.
 */
