...
describe('designerStore — multi-select keyframes', () => {
  it('openKeyframeInspector selects exactly one and opens the inspector', () => {
    designerStore.upsertKeyframe('el-1', 'position.x', 5, 50);
    designerStore.openKeyframeInspector({ elementId: 'el-1', property: 'position.x', frame: 5 });
    expect(designerStore.get().selectedKeyframes).toHaveLength(1);
    expect(designerStore.get().keyframeInspectorOpen).toBe(true);
  });

  it('addKeyframeToSelection adds and toggles points', () => {
    designerStore.upsertKeyframe('el-1', 'position.x', 5, 50);
    designerStore.upsertKeyframe('el-1', 'position.x', 20, 200);
    designerStore.openKeyframeInspector({ elementId: 'el-1', property: 'position.x', frame: 5 });
    designerStore.addKeyframeToSelection({ elementId: 'el-1', property: 'position.x', frame: 20 });
    expect(designerStore.get().selectedKeyframes.map((r) => r.frame).sort((a, b) => a - b)).toEqual([
      5, 20,
    ]);
    // toggling the same one off removes it
    designerStore.addKeyframeToSelection({ elementId: 'el-1', property: 'position.x', frame: 20 });
    expect(designerStore.get().selectedKeyframes.map((r) => r.frame)).toEqual([5]);
  });

  it('moving a point keeps it selected across the whole set', () => {
    designerStore.upsertKeyframe('el-1', 'position.x', 5, 50);
    designerStore.upsertKeyframe('el-1', 'position.x', 20, 200);
    designerStore.openKeyframeInspector({ elementId: 'el-1', property: 'position.x', frame: 5 });
    designerStore.addKeyframeToSelection({ elementId: 'el-1', property: 'position.x', frame: 20 });
    designerStore.moveKeyframe('el-1', 'position.x', 5, 8);
    expect(designerStore.get().selectedKeyframes.map((r) => r.frame).sort((a, b) => a - b)).toEqual([
      8, 20,
    ]);
  });

  it('removing a selected point drops it from the selection', () => {
    designerStore.upsertKeyframe('el-1', 'position.x', 5, 50);
    designerStore.upsertKeyframe('el-1', 'position.x', 20, 200);
    designerStore.openKeyframeInspector({ elementId: 'el-1', property: 'position.x', frame: 5 });
    designerStore.addKeyframeToSelection({ elementId: 'el-1', property: 'position.x', frame: 20 });
    designerStore.removeKeyframe('el-1', 'position.x', 5);
    expect(designerStore.get().selectedKeyframes.map((r) => r.frame)).toEqual([20]);
    expect(designerStore.get().keyframeInspectorOpen).toBe(true);
  });
});
