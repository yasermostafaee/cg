...
// `apps/designer/src/generated/cg-runtime.js`. The browser platform imports
// it as raw text (`?raw`) and feeds the bytes to the preview iframe + the
// export pipeline as the `cgJs` payload. The export emits the same payload
// inside every `.vcg`, so what the Designer shows IS what the Runtime plays.
//
// Mirrors `tools/template-fixtures/build.mjs`'s bundling step — the
// pattern is identical, so when the fixture builder evolves this script
// should follow.
...
const OUT_FILE = path.join(OUT_DIR, 'cg-runtime.js');
